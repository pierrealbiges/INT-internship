...
\newcommand{\FigureBenchPic}{
\begin{figure}[!h]
\centering{\includegraphics[width=\linewidth]{benchmark_fig.png}}
%\draw (0, .7\linewidth) node [above right=0mm] {$\mathsf{A}$};
\caption{(...)\label{fig:BenchPic}}
\end{figure}
}

\newcommand{\FigureNetworkPic}{
\begin{tikzpicture}[
squarednode/.style={rectangle, draw=black!60, fill=gray!5, very thick, minimum size=40mm}
]
\node[squarednode] (JUPYTER) {JUPYTER};
\node[squarednode] (GIT) [right=of JUPYTER] {GIT};

\draw[->] (JUPYTER.est) -- (GIT.west);
\draw[->] (GIT.west) -- (JUPYTER.est);
\end{tikzpicture}
}
